...
bool Gazua::API::access() {
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply] () {
        ...
        const auto tokenType = doc["token_type"].toString() == "Bearer" ? TokenType::BEARER : TokenType::UNKNOWN;
        const auto scopeStringList = doc["scope"].toString().split(',', QString::SkipEmptyParts);
        uint32_t scope = 0;
        ...
        if(doc["access_token"].isUndefined()) {
            m_token = std::nullopt;
            qDebug() << "Failed to fetch token";
        } else {
            const auto expiration = doc["expires_in"].toInt();
            const auto accessToken = doc["access_token"].toString();
            const auto refreshToken = doc["refresh_token"].toString();
